...
fun generateCommonFeedItemClickEvent(
    ...
    return FeedItemClickEvents(
        ...
        feedBottomEvents = FeedBottomEvents(
            ...
            onLike = {
                if (feedData.isLogin) feedData.onLike.invoke(feedData.feed.reviewId)
                else {
                    Log.i(tag, "로그인을 해주세요.")
            ...
            onFavorite = {
                if (feedData.isLogin) feedData.onFavorite.invoke(feedData.feed.reviewId)
                else {
                    Log.i(tag, "로그인을 해주세요.")
                    rootNavController.emailLogin()
                }
        ...
        onImage = {
            if (feedData.pageScrollable) rootNavController.imagePager(
                feedData.feed.reviewId,
                it
            )
        }, // TODO::[need optimize] 줌 상태에서 손을 때면 이미지 클릭 이벤트가 발생해 줌 하는동안은 pageScrollAble이 flase 상태여서 예외처리
